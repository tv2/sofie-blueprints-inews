		...
		if (cue) {
			...
			if (realBreaker) {
				return {
					transitionPrerollDuration: Number(config.studio.CasparPrerollDuration),
					transitionKeepaliveDuration:
						TimeFromFrames(Number(realBreaker.StartAlpha)) + Number(config.studio.CasparPrerollDuration),
					...
					expectedDuration: TimeFromFrames(
						Number(realBreaker.Duration) -
							Number(realBreaker.StartAlpha) -
							(Number(realBreaker.EndAlpha) + Number(config.studio.CasparPrerollDuration))
					)
				}
			}
		}
