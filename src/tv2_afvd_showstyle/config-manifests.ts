...
import { getGraphicsSetupsEntries, GRAPHICS_SETUPS_NAME_COLUMN_ID, GRAPHICS_SETUPS_TABLE_ID } from 'tv2-common'
...
export const gfxDesignTemplates: ConfigManifestEntry[] = [
	{
		...
		type: ConfigManifestEntryType.TABLE,
		required: true,
		// defaultVal: DEFAULT_GRAPHICS.map(val => ({ _id: '', ...val })).filter(template => template.IsDesign), // TODO C
		columns: [
			{
