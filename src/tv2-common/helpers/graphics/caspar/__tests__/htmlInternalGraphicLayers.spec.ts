...
describe('HTML internal graphics layering', () => {
	...
	it('includes headline in the HTML baseline reset slots', () => {
		...
		const headlineBaseline = baseline.find(obj => obj.layer === SharedGraphicLLayer.GraphicLLayerOverlayHeadline) as
			| TSR.TimelineObjCCGTemplate
			| undefined
		const compoundBaseline = baseline.find(obj => obj.layer === SharedGraphicLLayer.GraphicLLayerOverlay) as
			| TSR.TimelineObjCCGTemplate
			| undefined
